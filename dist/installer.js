...
export function runInstaller(options) {
    ...
    if (installAntigravity) {
        if (installForAntigravity())
            installedCount++;
    }
    if (installClaudeCode) {
        if (installForClaudeCode())
            installedCount++;
    }
    if (installCodex) {
        if (installForCodex())
...
function configHasServer(configPath) {
...
function codexCliHasServer() {
    if (process.env.TOKEN_SAVER_CODEX_CONFIG)
        return false;
    try {
...
export function getDoctorResult() {
...
function removeFromConfig(configPath) {
    ...
    try {
        const config = JSON.parse(readFileSync(configPath, "utf8"));
        if (!config.mcpServers || !Object.prototype.hasOwnProperty.call(config.mcpServers, "token-saver-mcp"))
            return false;
...
export function runUninstaller(options) {
    const isExplicit = options.antigravity || options.codex || options.claudecode;
    const isAll = options.all || !isExplicit;
    const paths = [
...
export function installForAntigravity(customConfigPath) {
...
export function installForClaudeCode(customConfigPath) {
