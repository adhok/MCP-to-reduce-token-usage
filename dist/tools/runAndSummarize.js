...
function testSummary(lines, kind) {
    ...
    if (kind === "pytest") {
        for (const line of lines) {
            ...
            if (match)
                failures.push(`- ${match[1]}${match[2] ? `: ${match[2]}` : ""}`);
        }
        for (const line of lines.filter((line) => /^E\s+/.test(line))) {
            if (failures.length > 0 && !failures.at(-1)?.includes(": "))
                failures[failures.length - 1] += `: ${line.replace(/^E\s+/, "")}`;
        }
    }
    else {
...
function tscSummary(lines) {
    const errors = lines.filter((line) => /(?:^|\s).*?(?:\(\d+,\d+\)|:\d+:\d+).*?error\s+TS\d+:/i.test(line));
    return errors.length
        ? { text: errors.join("\n"), wasSummarized: true }
        : genericSummary(lines);
}
function eslintSummary(lines) {
    const violations = [];
    let currentFile = "";
    for (const line of lines) {
        if (line.trim() && !/^\s*\d+:\d+\s/.test(line) && !/^\s*✖/.test(line))
            currentFile = line.trim();
        const match = line.match(/^\s*(\d+):(\d+)\s+(?:error|warning)\s+(.+?)\s+([\w/@.-]+)\s*$/);
        if (match)
            violations.push(`${currentFile}:${match[1]} ${match[4]}`);
    }
...
export function summarizeOutput(command, output) {
    ...
    if (/\bpytest(?:\s|$)/.test(commandName) || /\d+ passed.*\d+ failed/i.test(output))
        return testSummary(lines, "pytest");
    if (/\b(?:jest|vitest)(?:\s|$)/.test(commandName))
        return testSummary(lines, "jest");
    if (/\btsc(?:\.js)?(?:\s|$)/.test(commandName) || /error TS\d+:/i.test(output))
