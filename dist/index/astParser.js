...
function addSymbol(symbols, node, source, kind) {
...
function addClassMembers(symbols, node, source) {
    const body = bodyNode(node);
    if (!body)
        return;
...
function addObjectMembers(symbols, node, source) {
...
function addFunctionValue(symbols, node, source) {
    ...
    if (body?.type === "object")
        addObjectMembers(symbols, body, source);
}
function addTopLevelNode(symbols, node, source) {
    switch (node.type) {
        case "export_statement":
            for (const child of node.namedChildren)
                addTopLevelNode(symbols, child, source);
            return;
        ...
        case "variable_declaration":
            for (const child of node.namedChildren) {
                if (child.type === "variable_declarator")
                    addFunctionValue(symbols, child, source);
            }
            return;
        ...
        case "interface_declaration":
            ...
            for (const child of node.namedChildren.flatMap((child) => child.namedChildren)) {
                if (["method_signature", "method_declaration"].includes(child.type))
                    addSymbol(symbols, child, source, "method");
            }
            return;
