...
const hasInstallFlag = argv.some((arg) =>
  ["--install", "--antigravity", "--codex", "--claudecode", "--all", "-i"].includes(arg),
);
...
const packageJson = JSON.parse(readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "../package.json"), "utf8")) as { version: string };
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "run_command") {
    const args = (request.params.arguments ?? {}) as { command: string; cwd?: string; timeoutMs?: number; maxTokens?: number };
  ...
  if (request.params.name === "get_full_output") {
    const args = (request.params.arguments ?? {}) as { fullOutputKey: string };
  ...
  if (request.params.name === "prune_cache") {
    const args = (request.params.arguments ?? {}) as { maxAgeMs?: number };
  ...
  if (request.params.name === "clear_cache") {
    const args = (request.params.arguments ?? {}) as { confirm?: boolean };
    if (args.confirm !== true) throw new Error("clear_cache requires confirm: true.");
